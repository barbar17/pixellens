...
const PhotographyCard = ({ src, alt, title, index, href = "#" }: TPhotographyCard) => {
  return (
    <a href={href} className="relative overflow-hidden cursor-pointer aspect-2/3 block group">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex flex-col items-start px-2 md:px-8 pb-10">
        <div className="w-8 h-px bg-gold mb-4 scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-500" />
        <h3 className="font-heading text-lg md:text-[28px] font-light tracking-widest text-[#f5f0e8] mb-0 group-hover:mb-5 transition-all duration-400">
          {title}
        </h3>
        <div className="flex items-center gap-3 md:opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-400 delay-150">
          <span className="text-[8px] md:text-[10px] tracking-[0.3em] uppercase text-gold font-medium">
            Explore
          </span>
