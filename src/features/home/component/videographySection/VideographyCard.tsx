...
const VideographyCard = ({ href, src, alt, title, duration }: TVideographyCard) => {
  return (
    <a href={href} className="relative overflow-hidden cursor-pointer aspect-2/3 block group">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex flex-col items-start px-2 md:px-7 pb-8">
        <div
          className="w-8 h-px bg-gold mb-3.5 transition-all duration-500"
          style={{ clipPath: "inset(0 100% 0 0)" }}
        />
        <h3 className="font-heading text-lg md:text-[28px] font-light tracking-[0.06em] text-[#f5f0e8] italic mb-0 group-hover:mb-4 transition-all duration-400">
          {title}
        </h3>
        <div className="flex items-center gap-2 md:opacity-0 translate-y-2.5 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-400 delay-150">
          <div className="flex flex-row gap-2 text-[9px] md:text-[10px] tracking-[0.3em] uppercase text-gold font-medium">
